...
class SubscriptionListener:
    ...
    async def create_subscription_task(self):
        subscription_message = {"id": "1", "method": "subscribe", "params": {"channels": [f"orderbook.{instrument}.1.100" for instrument in self._instrument_results.keys()]}}
    ...
    async def get_message(self, instrument: str) -> Dict:
        ...
        while not self._instrument_results[instrument]:
            if retry_count == 5:
                raise ValueError(f"Failed to get message for {instrument} after {retry_count} retries. Please investigate.")
